...
class PlaceList(Resource):
    ...
    @api.header('Authorization', 'Bearer <your_token>', required=True)
    @jwt_required()
    def post(self):
        ...
        if place_data['owner_id'] != current_user['id']:
            return {'error': 'Unauthorized to create this place.'}, 403

        user = facade.user_repo.get_by_attribute('id', place_data['owner_id'])
        if not user:
